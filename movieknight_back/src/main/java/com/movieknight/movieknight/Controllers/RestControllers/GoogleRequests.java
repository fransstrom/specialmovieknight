...
public class GoogleRequests {
    ...
    private GoogleCredential getCredentials(User user) throws ParseException {
        String refreshToken;
        String accessToken;
        GoogleCredential credential;
    ...
    public void handleBooking(@RequestBody Booking booking) throws IOException, ParseException {
        ...
        for (User user : userList) {
            GoogleCredential credential = getCredentials(user);
            if (credential != null) {
                calendar = getCalendar(credential);
